...
export const GradesPage = () => {
    ...
    return (
        <div className={'w-full flex h-full flex-col md:flex-row gap-2'}>
            <div
                className={'hidden w-full h-full md:flex md:flex-1 bg-secondary rounded shadow p-3 gap-2 flex-col items-center'}>
                ...
                <div className={'h-full w-full overflow-y-auto flex flex-col gap-2'}>
                    {
                        grades.map((grade, index) => (
                            ...
                            >
                                ...
                                />
                                <h2 className={'font-medium'}>{grade.name}</h2>
                            </div>
                        ))
                    }
                </div>
                ...
                <div className={'w-full h-full grid grid-rows-2 md:grid-cols-2 gap-2'}>
                    {
                        grades.map(grade => (
                            <div
                                key={grade.id}
                                className={'bg-secondary rounded min-w-[33.33%] md:min-w-0 md:w-full gap-1 w-fit p-3 shadow flex flex-col md:h-min'}
                            >
