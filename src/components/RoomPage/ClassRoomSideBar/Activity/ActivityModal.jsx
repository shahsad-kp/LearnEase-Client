...
export const ActivityModal = ({closeFunction, activity}) => {
    return (
        <Modal closeFunction={closeFunction} classNames={' bg-secondary gap-2.5 p-4 rounded !max-w-[25%] max-h-[75%]'}>
            ...
            <div className={'h-full w-full flex flex-col gap-2'}>
                {
                    ...
                    activity.responses.map(response => {
                        return (
                            <div className={'w-full p-2 rounded flex flex-col ' + (response.isCorrect ? 'bg-green-300': 'bg-red-300')} key={response.id}>
